...
Plans are markdown documents. For optimal regex extraction, use these patterns:

- `**Goal:** <text>` — Goal extraction (may span multiple lines until blank line, heading, or next bold marker). Falls back to first `# heading`.
- `Create: \`path\`` / `Modify: \`path\`` — Key file extraction (also supports `Test:`, `Edit:`, `Update:`, `Delete:`).
- `### Task N: <title>` — Task counting (matches `### Task` followed by a number).

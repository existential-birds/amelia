...
- [ ] 06-03-PLAN.md -- Gap closure: thread head_branch parameter through orchestrator, fix fragile test assertion
...
### Phase 11: Fix Streaming Terminal Events & Pipeline Stage Emission
**Goal**: CLI commands `fix-pr` and `watch-pr` complete successfully by fixing the streaming terminal event set and adding stage event emission from PR auto-fix pipeline nodes
**Depends on**: Phase 7, Phase 9
**Requirements**: TRIG-01, TRIG-02
**Gap Closure:** Closes gaps from v1.0 audit
**Success Criteria** (what must be TRUE):
  1. `stream_workflow_events` terminates when PR auto-fix workflows complete or fail (terminal event set includes `pr_auto_fix_completed` and `pr_auto_fix_failed`)
  2. PR auto-fix pipeline nodes emit `stage_completed` events so `WorkflowSummary` counters reflect actual work done
  3. `fix-pr` CLI command runs to completion and exits cleanly
  4. `watch-pr` CLI loop advances past each `stream_workflow_events` call
**Plans:** 1/1 plans complete

Plans:
- [x] 11-01-PLAN.md -- Add PR auto-fix terminal events to streaming and emit stage_completed from orchestrator

### Phase 12: Wire Missing Events & Data Forwarding
**Goal**: All defined PR auto-fix event types are emitted at the correct points, API trigger forwards PR title, and frontend TypeScript types are complete
**Depends on**: Phase 11
**Requirements**: TRIG-03, DASH-01, DASH-03, DASH-04
**Gap Closure:** Closes gaps from v1.0 audit
**Success Criteria** (what must be TRUE):
  1. API trigger endpoint forwards `pr_summary.title` to orchestrator so dashboard shows real PR title
